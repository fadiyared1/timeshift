...
@section('content')
@if (session('success'))
   <div id="sucgreen" class="px-3 py-2 my-0.5 border border-green-800 rounded border-l-8 bg-green-200 text-sm text-green-700">
      {{ session('success') }}
   </div>
@endif
@if (session('error'))
  <div id="error" class="px-3 py-2 my-0.5 border border-red-800 rounded border-l-8 bg-red-200 text-sm text-red-700">
        {{ session('error') }}
  </div>
@endif
      ...
      <div class="shadow sm:rounded-md sm:overflow-hidden">
          <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
            ...
            Photo
              ...
              <form action="{{route('changej')}}" method="POST">
                @csrf
              <div>
                <h2 class="text-lg leading-6 font-medium text-gray-900">Change Job</h2>
            </div>
            <br>
            <div class="grid grid-cols-6 gap-6">
              <div class="col-span-6 sm:col-span-3">
                <label for="new_job" class="block text-sm font-medium leading-5 text-gray-700">Current Job</label>
                <input id="new_job" name="new_job" class="form-input mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5" value="{{$user->job}}" />
            </div>
            </div>
            <br>
