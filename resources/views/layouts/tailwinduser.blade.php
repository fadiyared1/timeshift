                  ...
                  <img class="h-8 w-8 rounded-full" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="" />
                        ...
                        <div id="dropdown" class="closed absolute origin-top-right hidden z-30 right-0 mt-2 w-48 rounded-md shadow-lg">
                            <div class="py-1 rounded-md bg-white shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                                <a href="{{route('user.newprofile')}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150" role="menuitem">Your Profile</a>
                    ...
                    <div id="sidebar-cont-user" class=" overflow-x-hidden flex flex-col flex-grow border-r border-gray-200 pt-5 pb-4 bg-white overflow-y-auto">
                        <nav class="px-2 bg-white">
                            <div class="space-y-1">
                                ...
                                <a href="{{route('user.projects')}}" class="group projects flex items-center px-2 py-2 text-sm leading-5 font-medium text-gray-600 rounded-md {{ (request()->is('user/projects')) ? 'bg-gray-100' : '' }} {{ (request()->is('user/projects')) ? 'text-gray-900' : '' }} hover:text-gray-900 hover:bg-gray-50 focus:outline-none focus:text-gray-900 focus:bg-gray-50 transition ease-in-out duration-150">
                                    <svg class="mr-6 h-6 w-6 flex-shrink-0 text-gray-400 {{ (request()->is('user/projects')) ? 'text-gray-500' : '' }} group-hover:text-gray-500 group-focus:text-gray-500 transition ease-in-out duration-150" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                                    <div class="menulabel whitespace-nowrap uppercase">Projects</div>
                                </a>

                                <a href="{{route('user.tags')}}" class="group tags flex items-center px-2 py-2 text-sm leading-5 font-medium text-gray-600 rounded-md {{ (request()->is('user/tags')) ? 'bg-gray-100' : '' }} {{ (request()->is('user/tags')) ? 'text-gray-900' : '' }} hover:text-gray-900 hover:bg-gray-50 focus:outline-none focus:text-gray-900 focus:bg-gray-50 transition ease-in-out duration-150">
                                    <svg class="mr-6 h-6 w-6 flex-shrink-0 text-gray-400 {{ (request()->is('user/tags')) ? 'text-gray-500' : '' }} group-hover:text-gray-500 group-focus:text-gray-500 transition ease-in-out duration-150" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path></svg>
                                    <div class="menulabel whitespace-nowrap uppercase">Tags</div>
                                </a>

                                <a href="{{route('user.newindex')}}" class="group task_tracker   flex items-center px-2 py-2 text-sm leading-5 font-medium text-gray-600 rounded-md {{ (request()->is('user/newindex')) ? 'bg-gray-100' : '' }} {{ (request()->is('user/newindex')) ? 'text-gray-900' : '' }}  hover:text-gray-900 hover:bg-gray-100 focus:outline-none focus:bg-gray-200 transition ease-in-out duration-150">
                                    <svg class="mr-6 h-6 w-6 flex-shrink-0 text-gray-400 {{ (request()->is('user/index')) ? 'text-gray-500' : '' }} group-hover:text-gray-500 group-focus:text-gray-500 transition ease-in-out duration-150" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>
