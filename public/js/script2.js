...
function append_users_actions_menu(el) {
    ...
    <div class="py-1 rounded-md bg-white shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
        <!--<a href="javascript:ShowHideedit('${value}')" class="block px-2 py-1 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150" role="menuitem">Edit</a>-->
        <a href="javascript:ShowHideres('${value}')" class="block px-2 py-1 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150" role="menuitem">Resend</a>
        <a href="javascript:ShowHidedel('${value}')" class="block px-2 py-1 text-sm text-red-700 font-semibold hover:bg-gray-100 transition ease-in-out duration-150" role="menuitem">Delete</a>
    </div>
    </div>`;
    }
    else{
...
function append_more_menu(el) {
    ...
    <div class="py-1 rounded-md bg-white shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
        ...
        <a href="javascript:DelTask('${value}')" class="block px-2 py-1 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150" role="menuitem">Delete</a>
    </div>
</div>`;
}
function DelTask(val){
    let popup = document.getElementById("delTask_popup");
    if (popup.classList.contains("hidden")) {
        popup.classList.remove("hidden");
        popup.getElementsByClassName("taskDelete")[0].setAttribute("href", "/user/"+val+"/taskdel");
    } else {
...
//     }
// }
function ShowHidedel(val) {
    let popup = document.getElementById("del_popup");
    $.post( "{{route('admin.test')}}", {id: val} );
    if (popup.classList.contains("hidden")) {
        popup.classList.remove("hidden");
        let str = val.split(",");
        //document.write(str[1]);
        popup.getElementsByClassName("confmsg")[0].innerHTML = str[1];
        popup.getElementsByClassName("actionDelete")[0].setAttribute("href", "/admin/"+str[0]+"/testudel");
    } else {
        popup.classList.add("hidden");
    }
...
function ShowHideres(val) {
    ...
    if (popup.classList.contains("hidden")) {
        ...
        let str = val.split(",");
        popup.getElementsByClassName("confmsg")[0].innerHTML = str[1];
        popup.getElementsByClassName("actionResend")[0].setAttribute("href", "/admin/"+str[0]+"/resend");
    } else {
        popup.classList.add("hidden");
    }
}
function ShowHideedit(val) {
    let popup = document.getElementById("edit_popup");
    
    if (popup.classList.contains("hidden")) {
        popup.classList.remove("hidden");
        let str = val.split(",");
        popup.getElementsByClassName("editformname")[0].setAttribute("value", str[1]);
        popup.getElementsByClassName("editformemail")[0].setAttribute("value", str[2]);
        popup.getElementsByClassName("editformjob")[0].setAttribute("value", str[3]);
        popup.getElementsByClassName("actionEdit")[0].setAttribute("value", str[0]);
    } else {
        popup.classList.add("hidden");
    }
}
